...
export const orderClient = {
  ...
  findByID: async (eventId: IdParam, orderId: IdParam) => {
    const response = await api.get<GenericDataResponse<Order>>(
  ...
  refund: async (
    eventId: IdParam,
    orderId: IdParam,
    refundPayload: RefundOrderPayload,
  ) => {
    const response = await api.post<GenericDataResponse<Order>>(
      "events/" + eventId + "/orders/" + orderId + "/refund",
      refundPayload,
    );
    return response.data;
  },
  ...
  resendConfirmation: async (eventId: IdParam, orderId: IdParam) => {
    const response = await api.post<GenericDataResponse<Order>>(
      "events/" + eventId + "/orders/" + orderId + "/resend_confirmation",
    );
    return response.data;
  },
  ...
  markAsPaid: async (eventId: IdParam, orderId: IdParam) => {
    const response = await api.post<GenericDataResponse<Order>>(
      "events/" + eventId + "/orders/" + orderId + "/mark-as-paid",
    );
    return response.data;
  },
  ...
  downloadInvoice: async (
    ...
    const response = await api.get(
      `events/${eventId}/orders/${orderId}/invoice`,
      {
        responseType: "blob",
      },
    );
...
export const orderClientPublic = {
  ...
  findOrderStripePaymentIntent: async (
    eventId: number,
    orderShortId: string,
  ) => {
  ...
  downloadInvoice: async (
    ...
    const response = await publicApi.get(
      `events/${eventId}/order/${orderShortId}/invoice`,
      {
        responseType: "blob",
      },
    );
